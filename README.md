...
工作区统一使用仓库根目录的 `.venv` 作为 Python 虚拟环境。

说明：

1. `.venv` 是本地开发环境目录，不纳入 Git 版本控制
2. 项目依赖以 `backend/requirements.txt` 为准，拉取仓库后请在本地自行创建 `.venv`
3. VS Code 新开的终端会优先激活仓库根目录的 `.venv`
4. 如果终端仍绑定旧环境，请关闭当前终端并重新打开后再执行命令
启动后可访问：
